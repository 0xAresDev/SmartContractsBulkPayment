...
import {Mixtral8x7BModelMarket} from "./LLMMarket.sol";
...
contract ProofOfStakeForumAI {
    ...
    constructor(address paymentTokenAddress, address _owner) {
        ...
        modelMarket = new Mixtral8x7BModelMarket(
            address(this),
            paymentTokenAddress
        );
        owner = _owner;
    ...
    function addNode(string memory url, uint256 price) external returns (bool) {
        require(
            paymentToken.allowance(msg.sender, address(this)) >= 1000 ether,
            "Not enough allowance!"
        );
        require(
            nodeStakers[msg.sender] == false,
            "Already staked with this address!"
        );
    ...
    function addValidator(address validator) external onlyOwner returns (bool) {
    ...
    function setWithdrawHoldTime(
        ...
        modelMarket.setWithdrawHoldTime(secs);
        return true;
    }
}
